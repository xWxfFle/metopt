...
def strongin_global_search(
    ...
    # Добавляем больше начальных точек для больших интервалов
    interval_length = right - left
    sampled = [(left, func(left)), (right, func(right))]
    
    if interval_length > 2.0:
        # Добавляем дополнительные точки равномерно по интервалу
        # Шаг между точками примерно 1.0 или меньше
        num_additional_points = max(2, int(interval_length))
        step = interval_length / (num_additional_points + 1)
        
        for i in range(1, num_additional_points + 1):
            x_point = left + i * step
            sampled.append((x_point, func(x_point)))
...
def render_report(
    ...
    # Сортируем точки по x для корректного построения ломаной
    samples_sorted = sorted(result.samples, key=lambda p: p[0])
    samples = np.array(samples_sorted)
    
    fig, ax = plt.subplots(figsize=(10, 6), dpi=150)

    ax.plot(xs, ys, label="f(x)", color="#1f77b4", linewidth=2)
    
    # Построение ломаной линии по отсортированным точкам
    ax.plot(
        samples[:, 0],
        samples[:, 1],
        marker="o",
